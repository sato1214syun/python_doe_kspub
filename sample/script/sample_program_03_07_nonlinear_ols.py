...
standard_regression_coefficients.to_csv(
    "sample/output/03_07/standard_regression_coefficients_nonlinear_ols.csv"
)
...
plt.savefig("sample/output/03_07/actual_vs_estimated_nonlinear_ols.png")
plt.show()  # 以上の設定で描画
...
results_train.to_csv(
    "sample/output/03_07/estimated_y_train_in_detail_nonlinear_ols.csv"
)
...
plt.ylabel("estimated y")  # y 軸の名前
plt.gca().set_aspect("equal", adjustable="box")  # 図の形を正方形に
plt.savefig("sample/output/03_07/actual_vs_estimated_nonlinear_ols_test.png")
plt.show()  # 以上の設定で描画
...
# 推定値を csv ファイルに保存。同じ名前のファイルがあるときは上書きされますので注意してください
results_test.to_csv("sample/output/03_07/estimated_y_test_in_detail_nonlinear_ols.csv")
