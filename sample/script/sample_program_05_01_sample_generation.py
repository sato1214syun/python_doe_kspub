...
setting_of_generation = pd.read_csv(
    "test_data/setting_of_generation.csv", index_col=0, header=0
)
...
if setting_of_generation.iloc[2, :].sum() != 0:
    for group_number in range(1, int(setting_of_generation.iloc[2, :].max()) + 1):
        ...
        actual_sum_of_components_converted = np.matlib.repmat(
            np.reshape(actual_sum_of_components, (x_generated.shape[0], 1)),
            1,
            len(variable_numbers),
        )
        x_generated[:, variable_numbers] = (
            x_generated[:, variable_numbers]
            / actual_sum_of_components_converted
            * desired_sum_of_components
        )
...
if setting_of_generation.shape[0] >= 4:
    for variable_number in range(x_generated.shape[1]):
        x_generated[:, variable_number] = np.round(
            x_generated[:, variable_number],
...
x_generated.to_csv("sample/output/05_01/generated_samples.csv")
