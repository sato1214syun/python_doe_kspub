        ...
        for j in range(x_tmp.shape[1]):
            if i == j:  # 二乗項
                ...
                x_prediction = pd.concat(
                    [
                        x_prediction,
                        x_prediction_square.rename(
                            columns={
                                x_prediction_square.columns[i]: "{0}^2".format(
                                    x_prediction_square.columns[i]
                                )
                            }
                        ).iloc[:, i],
                    ],
                ...
                x_prediction_cross = (
                    x_prediction_tmp.iloc[:, i] * x_prediction_tmp.iloc[:, j]
                )
...
if regression_method == "ols_linear" or regression_method == "ols_nonlinear":
    ...
    for nonlinear_svr_gamma in nonlinear_svr_gammas:
        gram_matrix = np.exp(
            -nonlinear_svr_gamma
            * ((autoscaled_x_array[:, np.newaxis] - autoscaled_x_array) ** 2).sum(
                axis=2
            )
        )
...
# 標準回帰係数
if (
    regression_method == "ols_linear"
    or regression_method == "ols_nonlinear"
    or regression_method == "svr_linear"
):
