...
for nonlinear_svr_gamma in ocsvm_gammas:
    gram_matrix = np.exp(
        -nonlinear_svr_gamma
        * ((autoscaled_x_array[:, np.newaxis] - autoscaled_x_array) ** 2).sum(axis=2)
    )
    variance_of_gram_matrix.append(gram_matrix.var(ddof=1))
optimal_gamma = ocsvm_gammas[
...
data_density_train.to_csv(
    "sample/output/04_02/ocsvm_gamma_optimization_data_density_train.csv"
)
...
inside_ad_flag_train.to_csv(
    "sample/output/04_02/inside_ad_flag_train_ocsvm_gamma_optimization.csv"
)
...
print(
    "\n予測用データセットにおける外れサンプル数 :",
    number_of_outliers_in_prediction_data,
)
print(
    "予測用データセットにおける外れサンプルの割合 :",
    number_of_outliers_in_prediction_data / x_prediction.shape[0],
)
data_density_prediction = pd.DataFrame(
    data_density_prediction, index=x_prediction.index, columns=["ocsvm_data_density"]
)
# csv ファイルに保存。同じ名前のファイルがあるときは上書きされるため注意
data_density_prediction.to_csv(
    "sample/output/04_02/ocsvm_gamma_optimization_data_density_prediction.csv"
)
...
inside_ad_flag_prediction.to_csv(
    "sample/output/04_02/inside_ad_flag_prediction_ocsvm_gamma_optimization.csv"
)
