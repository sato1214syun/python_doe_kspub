...
for sample_number in range(number_of_selecting_samples):
    ...
    for y_number in range(number_of_y_variables):
        ...
        if sample_number == 0:
            ...
            estimated_y = (
                autoscaled_estimated_y * y.iloc[:, y_number].std()
                + y.iloc[:, y_number].mean()
            )  # スケールをもとに戻す
            estimated_y_std = (
                autoscaled_estimated_y_std * y.iloc[:, y_number].std()
            )  # スケールをもとに戻す
            estimated_y = pd.DataFrame(
                estimated_y, index=x.index, columns=["estimated_y"]
            )
            ...
            print(
                ...
                mean_absolute_error(y.iloc[:, y_number], estimated_y),
            )
            ...
            print(
                "r^2 in cross-validation :",
                r2_score(y.iloc[:, y_number], estimated_y_in_cv),
            )
            print(
        ...
        estimated_y_prediction = (
            estimated_y_prediction * y.iloc[:, y_number].std()
            + y.iloc[:, y_number].mean()
        )
        estimated_y_prediction_std = (
            estimated_y_prediction_std * y.iloc[:, y_number].std()
        )
        ...
        std_of_estimated_y_prediction_all[:, y_number] = (
            estimated_y_prediction_std  # Y の予測値の標準偏差
        )
        probabilities_prediction_all[:, y_number] = (
            probabilities_prediction  # Y の目標達成確率
        )
    ...
    next_samples = pd.concat(
        [next_samples, x_prediction.loc[sum_of_log_probabilities.idxmax()]], axis=0
    )

    # x, y, x_prediction, cumulative_variance の更新
    x = pd.concat([x, x_prediction.loc[sum_of_log_probabilities.idxmax()]], axis=0)
    y = pd.concat(
        [y, estimated_y_prediction_all.loc[sum_of_log_probabilities.idxmax()]], axis=0
    )
    x_prediction = x_prediction.drop(sum_of_log_probabilities.idxmax(), axis=0)
    print(
        "sample number : {0} / {1}".format(
            sample_number + 1, number_of_selecting_samples
        )
    )
