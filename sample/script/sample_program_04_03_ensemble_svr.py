...
# 各サブデータセットで選択される説明変数の数の割合。0 より大きく 1 未満
...
x_prediction = pd.read_csv("test_data/resin_prediction.csv", index_col=0, header=0)
...
# 空の DataFrame 型の変数を作成し、ここにサブデータセットごとの y の推定結果を追加
estimated_y_train_all = pd.DataFrame()
...
for submodel_number in range(number_of_sub_datasets):
    ...
    optimal_svr_epsilon = svr_epsilons[np.where(r2cvs == np.max(r2cvs))[0][0]]
    ...
    for svr_c in svr_cs:
        ...
        r2cvs.append(r2_score(y, autoscaled_estimated_y_in_cv * y.std() + y.mean()))
    # クロスバリデーション後の r2 が最も大きい候補
    optimal_svr_c = svr_cs[np.where(r2cvs == np.max(r2cvs))[0][0]]
    ...
    for svr_gamma in svr_gammas:
        ...
        r2cvs.append(r2_score(y, autoscaled_estimated_y_in_cv * y.std() + y.mean()))
    # クロスバリデーション後の r2 が最も大きい候補
    optimal_svr_gamma = svr_gammas[np.where(r2cvs == np.max(r2cvs))[0][0]]
...
pd.to_pickle(
    ...
    "sample/output/04_03/selected_x_variable_numbers_svr_gaussian.bin",
)
pd.to_pickle(submodels, "sample/output/04_03/submodels_svr_gaussian.bin")
...
selected_x_variable_numbers = pd.read_pickle(
    "sample/output/04_03/selected_x_variable_numbers_svr_gaussian.bin"
)
submodels = pd.read_pickle("sample/output/04_03/submodels_svr_gaussian.bin")

# 予測用データセットの y の推定
estimated_y_prediction_all = pd.DataFrame()  # 空の DataFrame 型を作成し、ここにサブモデルごとの予測用データセットの y の推定結果を追加
for submodel_number in range(number_of_sub_datasets):
    # 説明変数の選択
    selected_autoscaled_x_prediction = autoscaled_x_prediction.iloc[
        :, selected_x_variable_numbers[submodel_number]
    ]
    # 予測用データセットの y の推定
    # 予測用データセットの y の値を推定し、Pandas の DataFrame 型に変換
    estimated_y_prediction = pd.DataFrame(
        submodels[submodel_number].predict(selected_autoscaled_x_prediction)
    )
    # スケールをもとに戻します
    estimated_y_prediction = estimated_y_prediction * y.std() + y.mean()
    estimated_y_prediction_all = pd.concat(
        [estimated_y_prediction_all, estimated_y_prediction], axis=1
    )
...
estimated_y_prediction.to_csv(
    "sample/output/04_03/estimated_y_prediction_ensemble_svr_gaussian.csv"
)
...
std_of_estimated_y_prediction.to_csv(
    "sample/output/04_03/std_of_estimated_y_prediction_ensemble_svr_gaussian.csv"
)
