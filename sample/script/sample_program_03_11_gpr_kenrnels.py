...
for index, kernel in enumerate(kernels):
    ...
    estimated_y_in_cv = np.ndarray.flatten(
        cross_val_predict(
            model, autoscaled_x_train, autoscaled_y_train, cv=cross_validation
        )
    )
...
plt.savefig("sample/output/03_11/actual_vs_estimated_y_train_gpr_kernels.png")
plt.show()  # 以上の設定で描画
...
results_train.to_csv("sample/output/03_11/estimated_y_train_in_detail_gpr_kernels.csv")
...
plt.ylabel("estimated y")  # y 軸の名前
plt.gca().set_aspect("equal", adjustable="box")  # 図の形を正方形に
plt.savefig("sample/output/03_11/actual_vs_estimated_y_test_gpr_kernels.png")
plt.show()  # 以上の設定で描画
...
# 推定値を csv ファイルに保存。同じ名前のファイルがあるときは上書きされますので注意してください
results_test.to_csv("sample/output/03_11/estimated_y_test_in_detail_gpr_kernels.csv")
