...
dataset = pd.read_csv("test_data/training_data_multi_y.csv", index_col=0, header=0)
x_prediction = pd.read_csv(
    "test_data/x_for_prediction_multi_y.csv", index_col=0, header=0
)
settings = pd.read_csv(
    "test_data/settings_in_bayesian_optimization_multi_y.csv", index_col=0, header=0
)
...
# Y の予測値の標準偏差を入れる変数
std_of_estimated_y_prediction_all = np.zeros(
    [x_prediction.shape[0], number_of_y_variables]
)
# Y の目標達成確率を入れる変数
...
for y_number in range(number_of_y_variables):
    ...
    if regression_method == "gpr_one_kernel":
        ...
        for index, kernel in enumerate(kernels):
            ...
            estimated_y_in_cv = (
                estimated_y_in_cv * std_of_y[y_number] + mean_of_y[y_number]
            )
    ...
    autoscaled_estimated_y, autoscaled_estimated_y_std = model.predict(
        autoscaled_x, return_std=True
    )
    # スケールをもとに戻す
    estimated_y = (
        autoscaled_estimated_y * y.iloc[:, y_number].std() + y.iloc[:, y_number].mean()
    )
    # スケールをもとに戻す
    estimated_y_std = autoscaled_estimated_y_std * y.iloc[:, y_number].std()
    estimated_y = pd.DataFrame(estimated_y, index=x.index, columns=["estimated_y"])
    estimated_y_std = pd.DataFrame(
        estimated_y_std, index=x.index, columns=["std_of_estimated_y"]
    )

    # トレーニングデータの実測値 vs. 推定値のプロット
    plt.rcParams["font.size"] = 18
    # 実測値 vs. 推定値プロット
    plt.scatter(y.iloc[:, y_number], estimated_y.iloc[:, 0], c="blue")
    # 実測値の最大値と、推定値の最大値の中で、より大きい値を取得
    y_max = max(y.iloc[:, y_number].max(), estimated_y.iloc[:, 0].max())
    # 実測値の最小値と、推定値の最小値の中で、より小さい値を取得
    y_min = min(y.iloc[:, y_number].min(), estimated_y.iloc[:, 0].min())
    ...
    plt.savefig(
        "sample/output/05_04/multi_y_actual_vs_estimated_{0}_{1}.png".format(
    ...
    print(
        ...
        root_mean_squared_error(y.iloc[:, y_number], estimated_y),
    )
    print(
        "MAE for training data :", mean_absolute_error(y.iloc[:, y_number], estimated_y)
    )
    ...
    results_train = pd.concat(
        [y_for_save, estimated_y, y_error_train, estimated_y_std], axis=1
    )
    # 推定値を csv ファイルに保存。同じ名前のファイルがあるときは上書きされますので注意してください
    results_train.to_csv(
        "sample/output/05_04/multi_y_estimated_y_in_detail_{0}_{1}.csv".format(
            regression_method, y_number
        )
    )
    ...
    estimated_y_in_cv = (
        autoscaled_estimated_y_in_cv * y.iloc[:, y_number].std()
        + y.iloc[:, y_number].mean()
    )  # スケールをもとに戻す
    ...
    plt.rcParams["font.size"] = 18
    # 実測値 vs. 推定値プロット
    plt.scatter(y.iloc[:, y_number], estimated_y_in_cv.iloc[:, 0], c="blue")
    # 実測値の最大値と、推定値の最大値の中で、より大きい値を取得
    y_max = max(y.iloc[:, y_number].max(), estimated_y_in_cv.iloc[:, 0].max())
    # 実測値の最小値と、推定値の最小値の中で、より小さい値を取得
    y_min = min(y.iloc[:, y_number].min(), estimated_y_in_cv.iloc[:, 0].min())
    # 取得した最小値-5%から最大値+5%まで、対角線を作成
    plt.plot(
        [y_min - 0.05 * (y_max - y_min), y_max + 0.05 * (y_max - y_min)],
        [y_min - 0.05 * (y_max - y_min), y_max + 0.05 * (y_max - y_min)],
    ...
    plt.gca().set_aspect("equal", adjustable="box")  # 図の形を正方形に
    plt.savefig(
        "sample/output/05_04/multi_y_actual_vs_estimated_in_cv_{0}_{1}.png".format(
            regression_method, y_number
        )
    )
    plt.show()  # 以上の設定で描画
    ...
    print(
        ...
        root_mean_squared_error(y.iloc[:, y_number], estimated_y_in_cv),
    )
    print(
        "MAE in cross-validation :",
        mean_absolute_error(y.iloc[:, y_number], estimated_y_in_cv),
    )
    ...
    results_in_cv.to_csv(
        "sample/output/05_04/multi_y_estimated_y_in_cv_in_detail_{0}_{1}.csv".format(
            regression_method, y_number
        )
    )
    ...
    estimated_y_prediction = (
        estimated_y_prediction * y.iloc[:, y_number].std() + y.iloc[:, y_number].mean()
    )
    ...
    if settings.iloc[0, y_number] == 1:
        probabilities_prediction = 1 - norm.cdf(
            max(y.iloc[:, y_number]) + y.iloc[:, y_number].std() * relaxation,
        ...
        probabilities_prediction = norm.cdf(
            min(y.iloc[:, y_number]) - y.iloc[:, y_number].std() * relaxation,
            loc=estimated_y_prediction,
            scale=estimated_y_prediction_std,
        )
    ...
    # Y の予測値の標準偏差
    std_of_estimated_y_prediction_all[:, y_number] = estimated_y_prediction_std
    # Y の目標達成確率
    probabilities_prediction_all[:, y_number] = probabilities_prediction
...
estimated_y_prediction_all = pd.DataFrame(
    estimated_y_prediction_all, index=x_prediction.index, columns=y.columns
)
# 予測結果を csv ファイルに保存。同じ名前のファイルがあるときは上書きされますので注意してください
estimated_y_prediction_all.to_csv(
    "sample/output/05_04/multi_y_estimated_y_prediction_multi_y_{0}.csv".format(
        regression_method
    )
)
std_of_estimated_y_prediction_all = pd.DataFrame(
    std_of_estimated_y_prediction_all, index=x_prediction.index, columns=y.columns
)
# 予測値の標準偏差を csv ファイルに保存。同じ名前のファイルがあるときは上書きされますので注意してください
std_of_estimated_y_prediction_all.to_csv(
    "sample/output/05_04/multi_y_estimated_y_prediction_multi_y_std_{0}.csv".format(
        regression_method
    )
)
probabilities_prediction_all = pd.DataFrame(
    probabilities_prediction_all, index=x_prediction.index, columns=y.columns
)
# 獲得関数 (目標達成確率) を csv ファイルに保存。同じ名前のファイルがあるときは上書きされますので注意してください
probabilities_prediction_all.to_csv(
    "sample/output/05_04/multi_y_probabilities_prediction_multi_y_{0}.csv".format(
        regression_method
    )
)
sum_of_log_probabilities = pd.DataFrame(
    sum_of_log_probabilities,
    index=x_prediction.index,
    columns=["sum_of_log_probabilities"],
)
sum_of_log_probabilities.to_csv(
    "sample/output/05_04/multi_y_sum_of_log_probabilities_prediction_multi_y_{0}.csv".format(
        regression_method
    )
)
...
next_sample.to_csv(
    "sample/output/05_04/multi_y_next_sample_bo_multi_y_{0}.csv".format(regression_method)
)
