...
plt.savefig(
    "sample/output/05_04/actual_vs_estimated_y_{0}.png".format(regression_method)
)
plt.show()  # 以上の設定で描画
...
results_train.to_csv(
    "sample/output/05_04/estimated_y_in_detail_{0}.csv".format(regression_method)
)
...
plt.gca().set_aspect("equal", adjustable="box")  # 図の形を正方形に
plt.savefig(
    "sample/output/05_04/actual_vs_estimated_y_in_cv_{0}.png".format(regression_method)
)
plt.show()  # 以上の設定で描画
...
results_in_cv.to_csv(
    "sample/output/05_04/estimated_y_in_cv_in_detail_{0}.csv".format(regression_method)
)
...
if acquisition_function == "MI":
    ...
    acquisition_function_prediction = norm.cdf(
        target_range[1], loc=estimated_y_prediction, scale=estimated_y_prediction_std
    ) - norm.cdf(
        target_range[0], loc=estimated_y_prediction, scale=estimated_y_prediction_std
    )
...
estimated_y_prediction.to_csv(
    "sample/output/05_04/estimated_y_prediction_{0}.csv".format(regression_method)
)
...
estimated_y_prediction_std.to_csv(
    "sample/output/05_04/estimated_y_prediction_{0}_std.csv".format(regression_method)
)
...
next_sample.to_csv(
    "sample/output/05_04/next_sample_bo_{0}_{1}.csv".format(
        regression_method, acquisition_function
    )
)
