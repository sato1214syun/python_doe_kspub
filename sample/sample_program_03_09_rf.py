...
variable_importances = pd.DataFrame(
    model.feature_importances_, index=x_train.columns, columns=["importances"]
)
# csv ファイルに保存。同じ名前のファイルがあるときは上書きされますので注意してください
variable_importances.to_csv("sample/output/03_09/variable_importances_rf.csv")
...
plt.savefig("sample/output/03_09/actual_vs_estimated_y_train_rf.png")
plt.show()  # 以上の設定で描画
...
results_train.to_csv("sample/output/03_09/estimated_y_train_in_detail_rf.csv")
...
plt.ylabel("estimated y")  # y 軸の名前
plt.gca().set_aspect("equal", adjustable="box")  # 図の形を正方形に
plt.savefig("sample/output/03_09/actual_vs_estimated_y_test_rf.png")
plt.show()  # 以上の設定で描画
...
# 推定値を csv ファイルに保存。同じ名前のファイルがあるときは上書きされますので注意してください
results_test.to_csv("sample/output/03_09/estimated_y_test_in_detail_rf.csv")
