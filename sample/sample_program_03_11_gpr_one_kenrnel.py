...
kernels = [
    ...
    ConstantKernel() * RBF(np.ones(x_train.shape[1]))
    + WhiteKernel()
...
estimated_y_train_std = (
    autoscaled_estimated_y_train_std * y_train.std()
)  # スケールをもとに戻す
estimated_y_train = pd.DataFrame(
    estimated_y_train, index=x_train.index, columns=["estimated_y"]
)
estimated_y_train_std = pd.DataFrame(
    estimated_y_train_std, index=x_train.index, columns=["std_of_estimated_y"]
)
...
plt.savefig("sample/output/03_11/estimated_y_train_gpr_one_kernel.png")  # 画像ファイルで保存
plt.show()  # 以上の設定で描画
...
results_train = pd.concat(
    [y_train_for_save, estimated_y_train, y_error_train, estimated_y_train_std], axis=1
)
# 推定値を csv ファイルに保存。同じ名前のファイルがあるときは上書きされますので注意してください
results_train.to_csv(
    "sample/output/03_11/estimated_y_train_in_detail_gpr_one_kernel.csv"
)
...
estimated_y_test_std = (
    autoscaled_estimated_y_test_std * y_train.std()
)  # スケールをもとに戻す
estimated_y_test = pd.DataFrame(
    estimated_y_test, index=x_test.index, columns=["estimated_y"]
)
estimated_y_test_std = pd.DataFrame(
    estimated_y_test_std, index=x_test.index, columns=["std_of_estimated_y"]
)
...
plt.ylabel("estimated y")  # y 軸の名前
plt.gca().set_aspect("equal", adjustable="box")  # 図の形を正方形に
plt.savefig("sample/output/03_11/estimated_y_test_gpr_one_kernel.png")  # 画像ファイルで保存
plt.show()  # 以上の設定で描画
...
results_test = pd.concat(
    [y_test_for_save, estimated_y_test, y_error_test, estimated_y_test_std], axis=1
)
# 推定値を csv ファイルに保存。同じ名前のファイルがあるときは上書きされますので注意してください
results_test.to_csv("sample/output/03_11/estimated_y_test_in_detail_gpr_one_kernel.csv")
